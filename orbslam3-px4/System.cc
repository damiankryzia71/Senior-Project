...
void System::ExportMapPointsToPLY(const std::string &filename)
{
    std::ofstream file(filename);
    auto mapPoints = mpAtlas->GetCurrentMap()->GetAllMapPoints();

    // Write PLY header
    file << "ply\nformat ascii 1.0\nelement vertex " << mapPoints.size()
         << "\nproperty float x\nproperty float y\nproperty float z\nend_header\n";
...
void System::Shutdown()
{
    ...
    if(!mStrSaveAtlasToFile.empty())
    {
        ...
        std::cout << "About to call SaveAtlas()..." << std::endl;
        SaveAtlas(FileType::BINARY_FILE);
        std::cout << "Finished SaveAtlas()" << std::endl;
    }
